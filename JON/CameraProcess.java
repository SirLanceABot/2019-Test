...
public class CameraProcess implements Runnable
{
	...
	public void run()
	{
		...
		// cameraHeight);
		outputStream = new CvSource("Bumper", VideoMode.PixelFormat.kMJPEG, 320, 240, 30);
		...
		while (!Thread.interrupted())
		{
			...
			if (debuggingEnabled)
			{
				System.out.println("[CameraProcess] " + 1.0 / (timer.get() - loopStartTime) + " FPS");
			}
			...
			if (Main.logImage)
			{
				try
				{
					...
					if (!Imgcodecs.imwrite(filename, mat))
					{
						System.out.println("Error writing B");
					}
				} catch (Exception e)
				{
					System.out.println("Error saving image file" + e.toString());
